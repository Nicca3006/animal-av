...
Damit das Rechteck wirklich *unter* dem Text liegt, setzen wir die `Tiefe` auf 16.
...
Das Fenster ist im Moment vielleicht leer, aber sobald du im _Animation Drawing Window_ (dem Fenster, in dem wir gerade gezeichnet haben), auf `Speichern` klickst, wird die "Animation" angezeigt. +
