...
footnote:[Meistens kannst nach _speichern unter_ einfach _demo.asu_ in den *Dateinamen* schreiben, egal welcher *Dateityp* voreingestellt ist].
...
Klicke dazu auf image:Open.PNG[] oder _Datei - Öffnen_ (_File - Open_) und wähle beim *Dateityp* _animation/animalscript(.asu).
...
Um zu erreichen, dass unser _text_ und _rectangle_ Befehl im gleichen Schritt ausgeführt werden, umgeben wir diese mit _geschweiften Klammern_.
...
Der zu einer *CodeGroup* gehörende Text kann durch folgende Eingabe hinzugefügt werden:
